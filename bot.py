...
async def help(ctx):
    ...
    helpEmbed.add_field(name=commandPrefix + "timer start [optional game name]", value="Only available in **Game Rooms** and **Campaigns**. Start a timer to keep track of time and rewards for games. Only one timer per game room can be active at once, and only the person who started the timer can stop it.")
    helpEmbed.add_field(name=commandPrefix + "timer stamp", value="Only available in **Game Rooms** and **Campaigns**. View the elapsed time on the timer running. Will also show the elapsed time for late players")
