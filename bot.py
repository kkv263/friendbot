...
async def help(ctx, *, pageString=''):
    ...
    helpEmbedChar.add_field(name=commandPrefix + 'respec "character name" "new character name" level "race" "class" "background" STR DEX CON INT WIS CHA', value="Respecs your character based on the hours your have played. Your character can choose new name, class, race, background, and stats. Any items gained during hours of play will be lost, can TP and GP will be allocated based on CP.", inline=False)
    helpEmbedChar.add_field(name=f'{commandPrefix}levelup "character name" [{commandPrefix}lvl, {commandPrefix}lv {commandPrefix}lvlup]', value="Levels your character to the next level. You may also choose to multiclass your character if you choose to.", inline=False)
    ...
    helpEmbedTimerOne.add_field(name=commandPrefix + "session log gameid summary", value="After a game a finished, the DM may edit the log and add a summary to the game in the session logs channel. After using this command to edit the log, the log will be checked and rewards will be given out.", inline=False)
